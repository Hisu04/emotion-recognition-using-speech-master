...
from core.utils import extract_feature, AVAILABLE_EMOTIONS
from core.create_csv import write_emodb_csv, write_tess_ravdess_csv, write_custom_csv
...
from core.utils import get_best_estimators, get_audio_config
import numpy as np
import tqdm
import os
import random
...
class EmotionRecognizer:
    ...
    def __init__(self, model=None, **kwargs):
        ...
        Params:
            ...
            tess_ravdess_name (str): the name of the output CSV file for TESS&RAVDESS dataset, default is "metadata/tess_ravdess.csv"
            emodb_name (str): the name of the output CSV file for EMO-DB dataset, default is "metadata/emodb.csv"
            custom_db_name (str): the name of the output CSV file for the custom dataset, default is "metadata/custom.csv"
        ...
        self.tess_ravdess_name = kwargs.get("tess_ravdess_name", "metadata/tess_ravdess.csv")
        self.emodb_name = kwargs.get("emodb_name", "metadata/emodb.csv")
        self.custom_db_name = kwargs.get("custom_db_name", "metadata/custom.csv")
    ...
    def _set_metadata_filenames(self):
        ...
        """
        import os
        def insert_prefix(prefix, filename):
            d, f = os.path.split(filename)
            return os.path.join(d, f"{prefix}_{f}") if d else f"{prefix}_{f}"
            
        train_desc_files, test_desc_files = [], []
        if self.tess_ravdess:
            train_desc_files.append(insert_prefix("train", self.tess_ravdess_name))
            test_desc_files.append(insert_prefix("test", self.tess_ravdess_name))
        if self.emodb:
            train_desc_files.append(insert_prefix("train", self.emodb_name))
            test_desc_files.append(insert_prefix("test", self.emodb_name))
        if self.custom_db:
            train_desc_files.append(insert_prefix("train", self.custom_db_name))
            test_desc_files.append(insert_prefix("test", self.custom_db_name))
