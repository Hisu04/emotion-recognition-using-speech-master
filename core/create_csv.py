...
def write_emodb_csv(emotions=["sad", "neutral", "happy"], train_name="metadata/train_emo.csv",
                    test_name="metadata/test_emo.csv", train_size=0.8, verbose=1):
    """
    ...
    params:
        ...
        train_name (str): the output csv filename for training data, default is 'metadata/train_emo.csv'
        test_name (str): the output csv filename for testing data, default is 'metadata/test_emo.csv'
...
def write_tess_ravdess_csv(emotions=["sad", "neutral", "happy"], train_name="metadata/train_tess_ravdess.csv",
                            test_name="metadata/test_tess_ravdess.csv", verbose=1):
    """
    Reads speech TESS & RAVDESS datasets from directory and write it to a metadata CSV file.
    params:
        emotions (list): list of emotions to read from the folder, default is ['sad', 'neutral', 'happy']
        train_name (str): the output csv filename for training data, default is 'metadata/train_tess_ravdess.csv'
        test_name (str): the output csv filename for testing data, default is 'metadata/test_tess_ravdess.csv'
        verbose (int/bool): verbositiy level, 0 for silence, 1 for info, default is 1
    """
...
def write_custom_csv(emotions=['sad', 'neutral', 'happy'], train_name="metadata/train_custom.csv", test_name="metadata/test_custom.csv",
                    verbose=1):
    """
    Reads Custom Audio data from data/*-custom and then writes description files (csv)
    params:
        emotions (list): list of emotions to read from the folder, default is ['sad', 'neutral', 'happy']
        train_name (str): the output csv filename for training data, default is 'metadata/train_custom.csv'
        test_name (str): the output csv filename for testing data, default is 'metadata/test_custom.csv'
        verbose (int/bool): verbositiy level, 0 for silence, 1 for info, default is 1
    """
    train_target = {"path": [], "emotion": []}
    test_target = {"path": [], "emotion": []}
